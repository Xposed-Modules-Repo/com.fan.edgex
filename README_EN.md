...
- Available actions include Back, Home, Recents, Expand Notifications, Lock Screen, Screenshot, Global Copy, Paste Clipboard, Kill Foreground App, Clear Background Apps, brightness control, volume control, launch app, app shortcuts, music control, and custom shell commands.
- Freezer support for freezing/unfreezing apps, storing a freezer list, opening a Freezer Drawer (Material You style) from gestures or keys, and refreezing apps in one step.
- Extra features include theme presets, custom RGB accent color, gesture debug overlay, haptic feedback on action trigger, action icons, and update checking.

Notes:

- The module currently targets Android 15+.
- LSPosed/Xposed scope only requires `android` (System Framework).
